...
/* #kbd-set1-shift: same story for SHIFT. QEMU 10 rejects the Set-2
 * switch ("[KBD] Set2 SWITCH FAILED"), so the guest runs raw Set-1
 * where the producer historically SWALLOWED modifier scancodes — the
 * consumer then mapped every character with a stale (empty) modifier
 * mask: capitals and all shifted punctuation (| > ? : " etc) were
 * impossible to type through burst input (sendkey/automation). The
 * consumer now tracks shift make/break IN RING ORDER, exactly like
 * Ctrl above; the producer pushes modifiers into the ring so the
 * order survives, and evdev's mirror stream sees them too.       */
static bool kb_getchar_shift = false;
...
char keyboard_getchar() {
    ...
    /* #kbd-set1-shift: stream-tracked shift (raw Set-1 only; the
     * Set-2 producer already tracks modifiers at IRQ time).        */
    if (!kbd_use_set2) {
        if (scancode == 0x2A || scancode == 0x36) {
            kb_getchar_shift = !released;
            return 0;                     /* modifier: never data      */
        }
    }
    ...
    u8 eff_mod = kbd_state.modifiers;
    if (!kbd_use_set2) {
        /* raw Set-1: the producer never maintained the mask — use
         * the consumer-streamed shift state instead.              */
        eff_mod = kb_getchar_shift ? KBD_SHIFT_PRESSED : 0;
    }
    char c = keyboard_scancode_to_char(scancode, eff_mod);
...
void keyboard_handler() {
    {
        extern void vmm_pml4_guard(int); vmm_pml4_guard(3);
    ...
    if (kbd_raw_mode) {
        ...
        { extern void evdev_kbd_set1(u8); evdev_kbd_set1(raw); }
        return;
    }
    ...
    if (!kbd_use_set2) {
        ...
        if (code == 0x1D) {                    /* Ctrl L               */
            s1_prod_ctrl = !brk;
            /* FIX(#evdev-modifier-mirror): Ctrl/Alt/Caps make+break
             * were never mirrored into the evdev stream (only Shift
             * was), so libinput/Wayland clients could not see any
             * modifier state — no Ctrl+C shortcuts in wl clients.
             * Mirror to evdev WITHOUT pushing into the console ring
             * (ring readers must not see bare modifiers). */
            { extern void evdev_kbd_set1(u8); evdev_kbd_set1(raw); }
            return;                            /* never data           */
        }
        if (code == 0x2A || code == 0x36) {    /* Shift L/R            */
            /* #kbd-set1-shift: update the mask for non-ring readers
             * AND push the event — the consumer tracks shift in ring
             * order, and the evdev mirror needs make/break pairs.  */
            if (!brk) kbd_state.modifiers |= KBD_SHIFT_PRESSED;
            else      kbd_state.modifiers &= ~KBD_SHIFT_PRESSED;
            keyboard_buffer_push(raw);
            { extern void evdev_kbd_set1(u8); evdev_kbd_set1(raw); }
            return;
        }
        if (code == 0x38) {                    /* Alt                  */
            if (!brk) kbd_state.modifiers |= KBD_ALT_PRESSED;
            else      kbd_state.modifiers &= ~KBD_ALT_PRESSED;
            { extern void evdev_kbd_set1(u8); evdev_kbd_set1(raw); }
            return;
        }
    ...
    /* evdev mirror stream (set-1 canonical form; E0/F0 prefix bytes
     * are filtered inside evdev) */
    if (!is_extended) {
        extern void evdev_kbd_set1(u8);
        evdev_kbd_set1(is_break ? (set1_scancode | 0x80) : set1_scancode);
    }
}

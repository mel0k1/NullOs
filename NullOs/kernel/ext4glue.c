 ...
 *
 * FIX(#ext4-path-overflow): all path building used unbounded
 * kstrcpy/kstrcat into 160-byte stack buffers with NO total-length
 * check — a crafted ext4 image with deep nesting or ~150-char names
 * overwrote the saved RBP/return address of this RECURSIVE ring-0
 * function (kernel stack smash). The builders below are strictly
 * bounded; an over-long path aborts that subtree instead of smashing
 * the stack. A depth cap keeps recursion sane too. */
#define EXT4_PATH_MAX 160
#define EXT4_ATTACH_DEPTH_MAX 8

/* bounded strcat: appends `s` to `dst` (capacity `cap`, always
 * NUL-terminated). Returns false when it did NOT fit. */
static bool ext4_path_append(char* dst, size_t cap, const char* s) {
    size_t dl = kstrlen(dst);
    size_t sl = kstrlen(s);
    if (dl + sl + 1 > cap) return false;
    kmemcpy(dst + dl, s, sl + 1);
    return true;
}

/* build "<base>/<name>" (with the slash only when needed) into
 * `out` — returns false when it does not fit. */
static bool ext4_path_join(char* out, size_t cap,
                           const char* base, const char* name) {
    if (kstrlen(base) + 1 + kstrlen(name) + 1 > cap) return false;
    kstrcpy(out, base);
    size_t bl = kstrlen(out);
    if (bl == 0) { kstrcpy(out, "/"); bl = 1; }
    if (out[bl - 1] != '/') { out[bl] = '/'; out[bl + 1] = 0; }
    return ext4_path_append(out, cap, name);
}

static u32 ext4_attach_tree_d(const char* epath, const char* rpath,
                              u32 depth);

static u32 ext4_attach_tree(const char* epath, const char* rpath) {
    return ext4_attach_tree_d(epath, rpath, 0);
}
...
static u32 ext4_attach_tree_d(const char* epath, const char* rpath,
    ...
    ext4_dir d;
    u32 attached = 0;
    char echild[EXT4_PATH_MAX];
    char rchild[EXT4_PATH_MAX];

    if (depth > EXT4_ATTACH_DEPTH_MAX) {
        serial_printf("[EXT4] attach: depth cap hit at %s\n", rpath);
        return 0;
    }
    ...
    for (;;) {
        ...
        /* ramfs child path (FIX(#ext4-path-overflow): bounded join) */
        if (!ext4_path_join(rchild, sizeof(rchild), rpath, echild))
            continue;                      /* over-long: skip subtree   */

        if (etype == EXT4_DE_DIR) {
            char ep[EXT4_PATH_MAX];
            if (!ext4_path_join(ep, sizeof(ep), epath, echild))
                continue;                  /* over-long: skip subtree   */
            fs_mkdir(rchild);
            attached += ext4_attach_tree_d(ep, rchild, depth + 1);
        } else if (etype == EXT4_DE_REG_FILE) {
            if (ext4be_count >= EXT4BE_MAX_FILES) break;
            char ep[EXT4_PATH_MAX];
            if (!ext4_path_join(ep, sizeof(ep), epath, echild))
                continue;                  /* over-long: skip entry     */

...
extern const unsigned char drmtest_elf_start[];
extern const unsigned char drmtest_elf_end[];
extern const unsigned char evtest_elf_start[];
extern const unsigned char evtest_elf_end[];
extern const unsigned char layertest_elf_start[];
extern const unsigned char layertest_elf_end[];
extern const unsigned char seatprobe_elf_start[];
extern const unsigned char seatprobe_elf_end[];
...
void kernel_init(void* mbi_ptr) {
    ...
    // Layer-2 dwl substrate devices: evdev synthesis (IRQ producers
    // live in the keyboard/mouse drivers) + DRM-lite card0 (reads the
    // VGA PCI BAR0 for the linear framebuffer base).
    evdev_init();
    drm_init();
    klog_info("[LAYER2] evdev + DRM-lite initialized\n");
    ...
    // from build/userland/*.elf via kernel/userprogs.S (.incbin).
    {
        ...
        }
        {
             ...
             * and the evdev (libinput substrate) event stream.      */
            const u32 drt_len = (u32)(drmtest_elf_end - drmtest_elf_start);
            if (drt_len > 0) {
                fs_write_file("/bin/drmtest", drmtest_elf_start, drt_len);
                klog_info("[FS] Registered /bin/drmtest (%u bytes)\n",
                          drt_len);
            }
            const u32 evt_len = (u32)(evtest_elf_end - evtest_elf_start);
            if (evt_len > 0) {
                fs_write_file("/bin/evtest", evtest_elf_start, evt_len);
                klog_info("[FS] Registered /bin/evtest (%u bytes)\n",
                          evt_len);
            }
            const u32 lt_len =
                (u32)(layertest_elf_end - layertest_elf_start);
            if (lt_len > 0) {
                fs_write_file("/bin/layertest",
                              layertest_elf_start, lt_len);
                klog_info("[FS] Registered /bin/layertest (%u bytes)\n",
                          lt_len);
            }
        }
        {
            /* Layer-3 chain probe: libseat-lite -> libinput-lite. */
            const u32 sp_len =
                (u32)(seatprobe_elf_end - seatprobe_elf_start);
            if (sp_len > 0) {
                fs_write_file("/bin/seatprobe",
                              seatprobe_elf_start, sp_len);
                klog_info("[FS] Registered /bin/seatprobe (%u bytes)\n",
                          sp_len);
            }
        }
        {
    ...
    // #procfs + #shm-open-path mountpoints. /proc/* nodes are
    // materialized on open (see proc_materialize in syscall.c);
    // /dev/shm hosts the POSIX-shm pool handles (musl shm_open).
    {
        fs_mkdir("/proc");
        fs_mkdir("/proc/self");
        fs_mkdir("/dev/shm");

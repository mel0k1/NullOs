...
static phys_addr_t carve_end  = 0;   /* zone end (exclusive)        */
static u64  carve_start_page = 0;

/* FIX(#pmm-carve-unbounded): the zone used to be a FIXED 2 MB at
 * 128 MB, but pmm_carve_pages() had no upper bound — on RAM > ~1.4GB
 * the refcount table walked past the reserved window while
 * pmm_alloc_page kept handing those same frames to user data. The
 * zone now grows on demand AND re-reserves itself in the bitmap, and
 * both the alloc-skip and the free-guard use carve_end (not a
 * hard-coded 2MB / an open-ended >= test). */
...
void pmm_free_page(phys_addr_t addr) {
    ...
    if (word_idx < bitmap_size) {
        u64 flags = spin_lock_irqsave(&pmm_lock);
        /* FIX(#pmm-free-never): the old guard rejected EVERY frame
         * >= 128 MB (no upper bound) instead of just the 2 MB carve
         * zone — on the 512 MB target all frames above 130 MB were
         * one-shot: exit/munmap/COW frees silently did nothing and
         * the PMM drained irreversibly under normal fork/exec churn. */
        if (!(carve_start_page && page_num >= carve_start_page &&
              page_num < carve_end / PAGE_SIZE))
...
void pmm_carve_init(u64 bytes) {
    ...
    carve_end  = carve_lo + npages * PAGE_SIZE;
...
phys_addr_t pmm_carve_pages(u64 count) {
    ...
    /* FIX(#pmm-carve-unbounded): growing past the current zone end
     * must ALSO reserve the freshly carved range in the bitmap,
     * otherwise the general allocator hands those frames to user
     * data while they hold page tables / COW refcounts. */
    if (carve_ptr + count * PAGE_SIZE > carve_end) {
        u64 extra_lo = carve_end;
        u64 extra_hi = carve_ptr + count * PAGE_SIZE;
        u64 extra_pages =
            ((extra_hi - extra_lo) + PAGE_SIZE - 1) / PAGE_SIZE;
        for (u64 p = extra_lo / PAGE_SIZE;
             p < extra_lo / PAGE_SIZE + extra_pages &&
             p < total_pages; p++) {
            bitmap[p / 64] |= (1ULL << (p % 64));
        }
        carve_end = extra_hi;
...
static bool pt_pool_take(phys_addr_t* out) {
    ...
    if (ok)
        serial_printf("[PTTAKE] p=%lx top=%d ra=%lx\n",
                      (unsigned long)*out, pt_pool_top,
                      (unsigned long)__builtin_return_address(0));
    return ok;
}
...
static bool pt_pool_give(phys_addr_t pa) {
    ...
    spin_unlock_irqrestore(&pt_pool_lock, fl);
    if (ok)
        serial_printf("[PTGIVE] p=%lx top=%d ra=%lx\n",
                      (unsigned long)pa, pt_pool_top,
                      (unsigned long)__builtin_return_address(0));
    return ok;
}
...
/* [PML4-GUARD] differential [511]-watch, callable from any context.
 * Prints ONLY on a change vs the last sampled value (per-site latch
 * is global on purpose: any transition is interesting). */
void vmm_pml4_guard(int site) {
    if (!kernel_pml4_phys) return;
    phys_addr_t ap = vmm_get_active_pml4();
    if (!ap || ap == kernel_pml4_phys) return;
    static u64 last = 0; static bool init = false;
    u64 v = ((u64*)PHYS_TO_VIRT(ap))[511];
    if (!init) { last = v; init = true; return; }
    if (v != last) {
        serial_printf("[W511] site=%d p=%lx [511]=%lx (was %lx)\n",
                      site, (unsigned long)ap, (unsigned long)v,
                      (unsigned long)last);
        u64* w = (u64*)PHYS_TO_VIRT(ap);
        serial_printf("[W511DUMP] q505=%lx q506=%lx q507=%lx q508=%lx\n",
                      (unsigned long)w[505], (unsigned long)w[506],
                      (unsigned long)w[507], (unsigned long)w[508]);
        serial_printf("[W511DUMP2] q509=%lx q510=%lx q511=%lx q0=%lx q1=%lx\n",
                      (unsigned long)w[509], (unsigned long)w[510],
                      (unsigned long)w[511], (unsigned long)w[0],
                      (unsigned long)w[1]);
        {
            extern const tss_t* tss_get(void);
            serial_printf("[TSSNOW] rsp0=%lx ist1=%lx\n",
                          (unsigned long)tss_get()->rsp0,
                          (unsigned long)tss_get()->ist1);
        }
        last = v;
    }
}
...
phys_addr_t vmm_get_phys(virt_addr_t virt) {
    ...
    /* FIX(#getphys-nx): non-executable leaves carry VMM_NX (bit 63);
     * returning it fused into the PA made every munmap/MAP_FIXED over
     * a data page call pmm_free_page(PA|NX) — page_num ≈ 2^51, out of
     * bitmap range, frame silently LEAKED while the mapping vanished. */
    return (pte & ~(0xFFFULL | VMM_NX)) | (virt & 0xFFF);
}

/* FIX(#uaccess-supervisor-hole): user_range_ok()'s static range check
 * can not tell a genuine USER page from the supervisor IDENTITY alias
 * that every process PML4 inherits below 1GB (kernel heap lives at PA
 * 16-80MB!) nor from the [2GB,4GB) MMIO alias. A ring-3 caller could
 * pass e.g. buf=0x2000000 and have the KERNEL read/write its own heap
 * on the caller's behalf (info leak / corruption). This walker vets
 * every 4K page of the range on the ACTIVE pml4:
 *   - present + VMM_USER           -> OK (genuine user page)
 *   - not present at any level     -> OK (demand pager will satisfy a
 *                                    ring-3 fault, matching existing
 *                                    OS semantics for user VAs)
 *   - present WITHOUT VMM_USER     -> REJECT (kernel alias / MMIO) */
bool vmm_range_is_user(virt_addr_t addr, u64 len)
{
    if (!kernel_pml4) return true;            /* early boot: no tables */
    u64* pml4 = active_pml4 ? active_pml4 : kernel_pml4;
    u64 first = addr & ~0xFFFULL;
    u64 last  = (addr + len - 1) & ~0xFFFULL; /* len>0 guaranteed by caller */
    for (u64 va = first; ; va += PAGE_SIZE) {
        ...
        if (pml4e & VMM_PRESENT) {
            ...
            if (pdpte & VMM_PRESENT) {
                if (pdpte & VMM_PS) {
                    if (!(pdpte & VMM_USER)) return false;
                } else {
                    u64* pd = (u64*)PHYS_TO_VIRT(pdpte & ~0xFFFULL);
                    u64 pde = pd[(va >> 21) & 0x1FF];
                    if (pde & VMM_PRESENT) {
                        if (pde & VMM_PS) {
                            if (!(pde & VMM_USER)) return false;
                        } else {
                            u64* pt = (u64*)PHYS_TO_VIRT(pde & ~0xFFFULL);
                            u64 pte = pt[(va >> 12) & 0x1FF];
        ...
        if (va == last) break;                /* avoid +4K overflow    */
        if (va > last) break;                  /* paranoid wrap guard   */
    }
    return true;
}

/* FIX(#audit-mmap-regression): leaf-ownership probe. MAP_FIXED
 * replacement must only release frames the PROCESS owned — i.e. a
 * present leaf WITH VMM_USER. The supervisor identity alias ([4MB,1GB)
 * VA==PA RAM window) and the [2GB,4GB) MMIO alias are inherited by
 * every process PML4; their frames belong to the kernel, and handing
 * them to pmm_free_page() re-sold live kernel RAM (double-alloc
 * corruption). Also returns false for non-present leaves so callers
 * never "free" a frame for a hole. */
bool vmm_leaf_is_user(virt_addr_t virt)
{
    if (!kernel_pml4) return false;
    u64* pml4 = active_pml4 ? active_pml4 : kernel_pml4;

    u64 pml4e = pml4[(virt >> 39) & 0x1FF];
    if (!(pml4e & VMM_PRESENT)) return false;
    u64* pdpt = (u64*)PHYS_TO_VIRT(pml4e & ~0xFFFULL);

    u64 pdpte = pdpt[(virt >> 30) & 0x1FF];
    if (!(pdpte & VMM_PRESENT)) return false;
    if (pdpte & VMM_PS) return (pdpte & VMM_USER) != 0;   /* 1GB leaf */

    u64* pd = (u64*)PHYS_TO_VIRT(pdpte & ~0xFFFULL);
    u64 pde = pd[(virt >> 21) & 0x1FF];
    if (!(pde & VMM_PRESENT)) return false;
    if (pde & VMM_PS) return (pde & VMM_USER) != 0;       /* 2MB leaf */

    u64* pt = (u64*)PHYS_TO_VIRT(pde & ~0xFFFULL);
    u64 pte = pt[(virt >> 12) & 0x1FF];
    return (pte & VMM_PRESENT) != 0 && (pte & VMM_USER) != 0;
}
...
bool vmm_handle_cow(virt_addr_t virt) {
    ...
    if (old & VMM_STICKY_RO) return false;   /* #mprotect-cow-mask:
                                              * pinned-RO is NOT COW —
                                              * let the PF killer take
                                              * the task down.       */
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (level == 0) {
            ...
            if (cow_user && (pte & VMM_USER)) {
                ...
                u64 nx = pte & VMM_NX;
                if (pte & VMM_STICKY_RO) {
                    /* #mprotect-cow-mask: pinned-RO page keeps its
                     * pin in BOTH spaces (no COW flag — the handler
                     * must refuse, not resolve). Refcounts stay
                     * consistent: mark_shared above already set 2. */
                    low |= VMM_STICKY_RO;
                } else {
                    low |= VMM_COW;
                }
                src[i] = pa | low | nx;   /* parent becomes RO too */
...
phys_addr_t vmm_fork_pml4(phys_addr_t src, bool cow_user) {
    ...
    serial_printf("[PML4NEW] p=%lx cow=%d [511]=%lx\n",
                  (unsigned long)child, cow_user ? 1 : 0,
                  (unsigned long)((u64*)PHYS_TO_VIRT(child))[511]);
    return child;
}

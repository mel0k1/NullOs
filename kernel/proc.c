...
static bool build_user_image(u64 pml4_phys, const void* data, u64 size,
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        cursor -= len;
        if (cursor < vec_total) return false;    /* overflow guard       */
    ...
    cursor -= vec_total;
    cursor &= ~15ULL;
    if (cursor < PAGE_SIZE) return false;        /* keep 4K for crt push */
    ...
    *av++ = 0;  *av++ = 0;
    ...
    for (u32 s = 0; s < nseg; s++) {
        ...
        for (u64 k = 0; k < seg[s].npages; k++) {
            ...
            if (!pte) { serial_printf("[BUILD] walk FAIL va=%lx\n", va); return false; }
    ...
    for (u64 k = 0; k < stack_np; k++) {
        u64* pte = vmm_walk_leaf(pml4_phys, stack_bot + k * PAGE_SIZE,
                                 true, true);
        if (!pte) { serial_printf("[BUILD] stack walk FAIL k=%lu\n", (unsigned long)k); return false; }
...
s32 process_spawn(const char* path, char* const argv[], int argc) {
    ...
    t->park_ksp  = NULL;
    t->started   = false;
    ksnprintf(t->name, TASK_NAME_MAX, "proc%d", t->pid);
...
s32 proc_do_fork(void) {
    ...
    ksnprintf(ch->name, TASK_NAME_MAX, "proc%d", ch->pid);
    sched_active_count_add();
...
s32 proc_do_wait4(s32 pid, u64 status_user, bool nohang, bool wuntraced) {
    ...
    for (;;) {
        ...
        if (found >= 0) {
            ...
            if (status_user && (status_user >= 0x400000ULL) &&
                (status_user < 0x7FFFFF000ULL))
